...
// @include        *localhost:600*/clan_stash.php*
// @include        *localhost:600*/charsheet.php*
// @include        *localhost:600*/account.php*
// @include        *localhost:600*/mallstore.php*
// @include        *localhost:600*/displaycollection.php*
// @include        *localhost:600*/clan_stash.php*
// @include        localhost:18481/charsheet.php*
// @include        localhost:18481/account.php*
// @include        localhost:18481/mallstore.php*
// @include        localhost:18481/displaycollection.php*
// @include        localhost:18481/clan_stash.php*
// @grant       GM_getValue
// @grant       GM_setValue
...
Misc. fixes and skill/familiar code added by Aetheri
This is a branched version with random fixes and tweaks by antimarty
  - itemize stash
  - highlight bargain items when browing stores
  - track items in store
  - etc.
*/

// var currentVersion = 0.672;
var currentVersion = 0.999;  // marty - disable update unless a new one
...
prices = new Object();
items = new Object();
familiars = new Object();
useFamiliars = GM_getValue('useFamiliars',false);
skills = new Object();
useSkills = GM_getValue('useSkills',false);
...
presetLocations = {"Revised Pricelist" : "http://sites.google.com/site/pricegunrepository/files/dailyprice.log",
					"LogiKoL: Coldfront" : "numero.logikol.com/coldfront.txt",
					"LogiKoL: The Items of Loathing" : "numero.logikol.com/iol.txt",
					"LogiKoL: KoLmafia" : "numero.logikol.com/mafia.txt",
					"LogiKoL: Lo Mein" : "numero.logikol.com/lomein.txt",
					"LogiKoL: Hi Mein" : "numero.logikol.com/himein.txt"};
versionLocation = "sites.google.com/site/pricegunrepository/files/VERSION";
skillLocation = "sites.google.com/site/pricegunrepository/files/skilldata.txt";
familiarLocation = "sites.google.com/site/pricegunrepository/files/familiardata.txt";

// marty addition, security manager didn't like it without this
var inventoryTotal;
var closetTotal;
var hagnksTotal;
var dcTotal;
...
var smallCss = "span.meatAmt {	margin-right:4em;	color:white;}"+
	"a.dcLink {	text-decoration:none;	color:white;}"+
	"a.dcLink:hover,span.meatAmt:hover {	cursor:pointer;	color:#dedede;}"+
	"a.dcLink:visited {	color:white;}"+
"span.subtotalLabel {	color:white;	width:5em;	margin-right:1em;	font-family: \"arial black\", sans-serif;	font-weight: normal;}"+
"/* Display case styles */"+
	"td.leaderLine span { 	background-color:white; 	padding-right:2px;} "+
	"td.leaderLineRight {	font-size:10pt;	color:#888;	vertical-align: bottom; }"+
	"td.leaderLine { 	padding-left:0;	padding-right:1em;	vertical-align: bottom; "+
		"background-image : url(data:image/gif;base64,R0lGODlhAwAEAID%2FAJkzZsDAwCH5BAEAAAEALAAAAAADAAQAAAIEDIKpBQA7); "+
		"background-repeat: repeat-x; 	background-position: bottom; } "+
	"span.shelfValueSpan {	display:block;	font-size:9pt;	font-weight: bold; } "+
"/* -- shelf readout -- */"+
	"table#shelfList {	color:#66626e;	border-left:3px solid #8a7aa1;	border-right:3px solid #8a7aa1;	font-size:9pt;	width:100%;}"+
	"table#shelfList a.shelfLink {	color:#66626e;	text-decoration:none;}"+
	"table#shelfList a.shelfLink:hover {	color:black;}"+
	"table#shelfList td {	padding:5px 3px 4px 5px;}"+
	"tr.odd td {	background-color:#efedf1;}"+
	"tr.even td {	background-color:#faf8fc;}"+

	"div#linkDiv, div#storePrice, div#stashPrice {	-moz-border-radius-bottomleft: 3em;	-moz-border-radius-bottomright: 3em;}"+
	"div#total, div#storeTotal, div#stashTotal  {	-moz-border-radius-topleft: 3em;	-moz-border-radius-topright: 3em;}"+
	"div#total,div#linkDiv,div#storeTotal,div#stashPrice,div#storePrice,div#stashTotal {	text-align:center;	min-height:1.5em;	padding:.3em .5em .3em .5em;	background-color:#8a7aa1;}"+
	"div#storeInfo,div#stashInfo {	background-color:#efedf1;	color:#66626e;	font-size:9pt;	text-align:center;	padding:.3em 0.3em 0;	border-left:3px solid #8a7aa1;	border-right:3px solid #8a7aa1;}"+
	"div#shelfListHolder {	max-height:400px;	overflow:auto;	padding:0;}"+
	"div#infoHolder {	font-size:9pt;	width:400px;	padding:5px 5px 8px 10px;	margin-bottom:20px;	text-align:left;}"+
"/* --tooltip stuff-- */"+
	"td.tooltipCell {	cursor:help;}"+
	"td.tooltipCell:hover > span.tooltipHolder {	background-color:#8a7aa1;	color:white;}"+
	"td.tooltipCell span.tooltipHolder {"+
"	/* For popup positioning */"+
	"position:relative;}"+

	"td.tooltipCell:hover span.tooltipSpan {	display:block;}"+
	"td.tooltipCell  span.tooltipSpan {	display:none;	font-weight: bold;	white-space:nowrap;	top:2em;	right:-5em;	position:absolute;	background-color:#705a8f;	border:3px solid #cbcacd;	-moz-border-radius: 2em;	-moz-opacity:0.8;	color:white;	padding: 6px;	font-size:9pt;}";
...
function setDataLocation() {
	var newDataLocation  = prompt('Set new price file location:\n',dataLocation);
	if (newDataLocation==null)
		return;
	dataLocation=newDataLocation;
	GM_setValue('dataLocation',dataLocation);
}
...
function buildSettings() {
	...
	var priceListLocation = dataLocation;
	prefLink.setAttribute('href','javascript:void(0)');
	prefLink.setAttribute('id','setdatalocation');
    prefLink.innerHTML = ("Change location of price list");
	prefLink.addEventListener("click", setDataLocation, true);
...
/*	   
	...
	//create button to toggle location input
	toggleUpdateLocation = document.createElement('span');
	updateLocation = document.createElement('input');
	updateLocationButton = document.createElement('input');
	locationSelector = document.createElement('select');
	with(toggleUpdateLocation) {
			style.cursor = "pointer";
			appendChild(document.createTextNode('Change location of price list'));
			style.textDecoration = "underline";
			className = "locationButton";
			addEventListener('click',function(evt) {toggle(updateLocation);
	...
	with(updateLocationButton){
		style.display = "none";
		type = "button";
		className = "button";
		value = "Change Location";
		addEventListener('click',function(evt){dataLocation = updateLocation.value;
												GM_setValue('dataLocation', updateLocation.value);
												evt.preventDefault();},true);
	}
	...
	with(locationSelector){
		...
		defaultOption.textContent = 'Choose a location or enter one manually';
		appendChild(defaultOption);
		for(var loc in presetLocations){
			option = document.createElement('option');
			option.value = presetLocations[loc];
			option.textContent = loc;
			appendChild(option);		
		}
		addEventListener('change',function(evt){var newValue = this.options[this.selectedIndex].value;
												updateLocation.value = (newValue == 'none') ? updateLocation.value : newValue;} ,true);
	}
	...
	updateFamiliars = document.createElement('input');
	with(updateFamiliars){
		type = "checkbox";
		if(useFamiliars){
			checked = "checked";
		}
	...
	updateSkills = document.createElement('input');
	with(updateSkills){
		type = "checkbox";
		if(useSkills){
			checked = "checked";
	...
	updateConsumption = document.createElement('input');
	with(updateConsumption){
		type = "checkbox";
		if(useConsumption){
			checked = "checked";
		}
	...
	familiarPref = document.createElement('div');
	with(familiarPref){
		style.textAlign = "center";
		style.maxWidth = "15em";
		appendChild(updateFamiliars);
		appendChild(document.createTextNode("Count familiars in terrarium towards total meat."));
	}

	skillPref = document.createElement('div');
	with(skillPref){
		style.textAlign = "center";
		style.maxWidth = "15em";
		appendChild(updateSkills);
		appendChild(document.createTextNode("Count skills gained from items towards total meat."));
	}

	consumptionPref = document.createElement('div');
	with(consumptionPref){
		style.textAlign = "center";
		style.maxWidth = "15em";
		appendChild(updateConsumption);
		appendChild(document.createTextNode("Count food and drink consumed towards total meat."));
	}
	...
	with(prefWrapper) {
		...
		appendChild(toggleUpdateLocation);
		appendChild(document.createElement('br'));
		appendChild(updateLocation);
		appendChild(locationSelector);
		appendChild(updateLocationButton);
		appendChild(document.createElement('br'));
		appendChild(familiarPref);
		appendChild(document.createElement('br'));
		appendChild(skillPref);
		appendChild(document.createElement('br'));
		appendChild(consumptionPref);
	}
	
	//find area to append the whole mess to (insert just above the "until rollover" clock)
	...
	*/
	...
	/* ------------
	   ...
	   ------------ */
		...
		dcList = new Array();
		stashList = new Array(); // marty
		familiarList = new Array();
		skillList = new Array();
		consumptionList = new Array();
		activetab = 0;
		
		playerName=document.getElementsByTagName('a')[1].textContent;  // marty-6
...
function checkCurrentStore() {
	...
	for(var i=0;i<s.length;i++) {
		if(s[i].type == "radio") {
			...
			if(itemPrice==999999999)itemPrice=0; if(itemPrice==0) itemValue = 0; // marty - just ignore stuff at max price
			totalValue += itemValue*itemQ
			priceCell.title = "Diff: " +addCommas(itemPrice-itemValue)+" meat";
			priceCell.style.cursor = "help";
			
			// marty - this is where to check the value
			var veryLowPriceColor = "red";
			var veryLowPriceThreshold = .25;
			var lowPriceColor = "blue";
			var lowPriceThreshold = .1;
			var zeroPriceColor = "green";
			var ignorePriceColor = "silver"
			var ignorePriceThreshold = 999999000.0;
			
			if(itemPrice > ignorePriceThreshold || itemPrice == 0) {
//			    alert("hit the marty overpriced code!");
                row.cells[2].style.color =  ignorePriceColor;
                row.cells[3].style.color =  ignorePriceColor;
			}
			else if(itemValue == 0.0 ) {
//			    alert("hit the marty zero price code!");
                row.cells[2].style.color =  zeroPriceColor;
			}
			else if((itemValue-itemPrice)/itemValue > veryLowPriceThreshold) {
//			    alert("hit the marty very low price code!");
                row.cells[2].style.color =  veryLowPriceColor;
			}
			else if((itemValue-itemPrice)/itemValue > lowPriceThreshold) {
//			    alert("hit the marty low price code!");
                row.cells[2].style.color =  lowPriceColor;
			}
			// end marty
...
//			var martytemp=0;
			for(var b=0;b<a.length;b++) {
				...
				if((clk = a[b].getAttribute('onclick')) != null && clk.indexOf('descitem') != -1) {
					// marty change - descitem() added a ",playerid" that needs to be eliminated
...
//						if(martytemp < 3) {
//							alert(descNum +" couldn't be found in itemdatabase (DC)"); // MARTYTEMP
//							martytemp = martytemp+1;
//						}
					}
...
function doCloset() {
	...
	function pushCloset(details) {
		...
		if(typeof closetList != "undefined") closetList.sort(sortbyTotal);
		...
		// marty - move to DC, where there is still an addgoodies form
...
//		inventoryList = returned.list;
		...
		status("checking current equipment");
		...
		for(var i=0;i<a.length;i++) {
				...
				if((clk = a[i].getAttribute('onclick')) != null && clk.indexOf('descitem') != -1) {
					...
					if((theItem=items[descNum])) {
						...
						GM_log(descNum +" couldn't be found in itemdatabase");
//						alert(descNum +" couldn't be found in itemdatabase (current equip)"); // MARTYTEMP
						var val=0,total=0;
			...
			for(var i=0;i<a.length;i++) {
				...
				if((clk = a[i].getAttribute('onclick')) != null && clk.indexOf('descitem') != -1) {
					...
					if((theItem=items[descNum])) {
						...
						GM_log(descNum +" couldn't be found in itemdatabase");
//						alert(descNum +" couldn't be found in itemdatabase (familiars)"); // MARTYTEMP
						var val=0,total=0;
						//itemName = a[i].parentNode.nextSibling.firstChild.textContent;
						var imgSrc = a[i].src;
			...
			if(useFamiliars){
				status("checking terrarium", true);
				var b = parser.getElementsByTagName('img');
				for(var j = 0; j < b.length; j++){	
					var clk;
					if((clk = b[j].getAttribute('onclick')) != null && clk.indexOf('fam') != -1) {
						var currentFamiliar = familiars[/fam\((\d*)\)/.exec(clk)[1]]
						if(currentFamiliar != null){
							var val = (val = prices[currentFamiliar['itemID']]) ? val[0] : 0;
							familiarTotal += val * 1;
							familiarList.push([currentFamiliar['name'], 1, val, val, currentFamiliar['itemID'], false]);
						}
					}
				}
				status("sorting familiar list");
				familiarList.sort(sortbyTotal);
			}
		}
		// marty - moved to DC since not done with inventory yet
...
function doHagnks() {
	...
	// marty - use the gift shop hagnk's list
//	GM_get(document.location.host + "/storage.php",pushHagnks);
	GM_get(document.location.host + "/town_sendgift.php",pushHagnks);
...
//		if(hagnksMeat = /You have (\d*) meat in long-term storage/.exec(details.replace(/,/g,''))){hagnksMeat=hagnksMeat[1]};
		if(hagnksMeat = /You have (\d*) Meat in Hagnk/.exec(details.replace(/,/g,''))){hagnksMeat=hagnksMeat[1]};
		if(hagnksMeat*1>0) {
			hagnksTotal+=hagnksMeat*1;
			hagnksList.push(["Meat",,,hagnksMeat,]);
		}
		returned = parseFromForm1('giftsend','hagnks_whichitem1', hagnksTotal,hagnksList);
...
function doConsumption(){
	...
	consumptionTotal = 0;
	status("checking consumption data", true);
	GM_get(document.location.host + "/showconsumption.php",pushConsumption);
	function pushConsumption(details){
		parser.innerHTML = details;
		var a = parser.getElementsByClassName('nounder');
	   	for(var i = 0; i < a.length; i++){
			if(a[i].hasAttribute('href')){
				var descNum = /\((\d+)\)/.exec(a[i].getAttribute('href'));
				if(descNum != null){
					var currentItem;
					descNum = descNum[1];
					if((currentItem = items[descNum])){
						var val = (val = prices[currentItem[0]]) ? val[0] : 0;
						var num = parseInt(a[i].parentNode.nextSibling.textContent);
						consumptionTotal += val * num;
						consumptionList.push([currentItem[1], num, val, val * num, currentItem[0], false]);
					}
//					else
//						alert(descNum +" couldn't be found in itemdatabase (consumption)"); // MARTYTEMP						
				}
			}
		}
		status("sorting consumption list");
		consumptionList.sort(sortbyTotal);
		doDC();
	}
}
...
function doDC() {
	...
	function pushDC(details) {
		...
		// new location for inventory parsing (moved from closet) - Marty
		var tempList = [];
		returned = parseFromForm('addgoodies',0,tempList);
		inventoryTotal += returned.total;
		merge(inventoryList, returned.list);
		
		// marty - moved here from above, since inventory was delayed
		status("sorting inventory list");
		inventoryList.sort(sortbyTotal);
...
function doSkills(){
	...
	skillTotal = 0;
	status("checking skills", true);
	GM_get(document.location.host + "/charsheet.php",pushSkills);
	function pushSkills(details){
		parser.innerHTML = details;
		...
		for(var i = 0; i < a.length; i++){
			if(a[i].hasAttribute('onclick')){
				...
				if(skillID in skills){
					...
					skillTotal += val * 1;
					skillList.push([skills[skillID]['name'], 1, val, val, skills[skillID]['itemID'], false]);
				}
			}
		}
...
function doOtherSkills(){
	...
	function pushOtherSkills(details){
		parser.innerHTML = details;
		var a = parser.getElementsByTagName('select');
		for(var i = 0; i < a.length; i++){
			if(a[i].hasAttribute('name') && a[i].getAttribute('name') == "whichskill"){
				var b = a[i].getElementsByTagName('option');
				for(var j = 0; j < b.length; j++){
					if(b[j].hasAttribute('value')){
						var skillID = b[j].getAttribute('value');
						if(skillID in skills){
							var foundSkill = false;
							for(var k = 0; k < skillList.length && !foundSkill; k++){
								if(skillList[k][4] == skills[skillID]['itemID']){
									foundSkill = true;
								}
							}
							if(!foundSkill){
		...
		status("sorting skill list");
		skillList.sort(sortbyTotal);
...
// marty function
function doStash() {
//alert("entering doStash");				

	stashTotal=0;
	status("checking stash",true);
	GM_get(document.location.host + "/clan_stash.php",pushStash);
	
	function pushStash(details) {
// alert("entering pushStash");				
		parser.innerHTML = details;
		...
		var fs,f,opts;
		if((fs=parser.getElementsByTagName('form')) && (f=fs.namedItem('takegoodies')) && (opts=f.elements.namedItem('whichitem').options)) {
			itemTypes=opts.length-1;
			for(var i=1,l=opts.length;i<l;i++) {
				var id,info,q,itemName,val,total,static;
//				var id,info,q,total;
				id=opts[i].value;
				info=opts[i].textContent;
// alert("checking stash item, text = " + info);				
				if(q=/\((\d*)\)/.exec(info))q=q[1];else q=1;
				if(!(val = prices[id])){val=0}else{val=val[0]}
				if(itemName = /(.*?) \(/.exec(info)) itemName=itemName[1];else itemName =info;  // the .*? is the non-greedy version of .*
				total=q*val;
				itemQuantities+=q*1
				stashTotal+=total;
				stashList.push([itemName,q,val,total,id]);
			}
		}
		status("sorting stash list",false);
		stashList.sort(sortbyTotal);
...
// 2013-10-06 updated for new store interface
function doStore() {
	storeTotal=0;
...
function generateNewGroups() {
	var listArray = ["inventory","closet","store","hagnks","dc","stash","familiar","skill","consumption"];
	storedLists=eval(GM_getValue("lastLists_"+playerName,[]));
...
function storeCurrentLists() {
		GM_setValue("lastLists_"+playerName,[inventoryList,closetList,storeList,hagnksList,dcList,stashList,familiarList,skillList,consumptionList].toSource());
		generateReadout();
}
function newCheck() {
	//clear variables?
...
function generateReadout() {
	...
	{  // marty
		listArray.push(["Stash", "stash"]);
		windowSize += 5;
	}
	
	readoutWindow.style.width = windowSize + "em";
	...
	// marty - this is kind of lame, but oh well
	// make a single list of everything in the store, since I can't figure out an
	// efficient way to search the multi-dimensional array
	var localStoreList = new Array();
	for(var m=0; m<storeList.length;m++) {
		localStoreList.push(storeList[m][0]);
	}
	// end marty
...
//				totalWorth+=sections[w]*1;
				if(w < sections.length - 1) totalWorth+=sections[w]*1; //marty - don't include clan stash (last section)
...
//			itemCell.className = "tableHeader";
			...
			// marty
			if(i>0 && i != 3) {
				var starCell=headerRow.appendChild(document.createElement('th'));
				starCell.appendChild(document.createTextNode(' '));
				starCell.addEventListener('click',sortByColumn,true);
				starCell.className = "tableHeader";

				var itemCell=headerRow.appendChild(document.createElement('th'));
				itemCell.appendChild(document.createTextNode('Item (* if in store)'));
				itemCell.addEventListener('click',sortByColumn,true);
				itemCell.className = "tableHeader";
			}
			//end marty
	  ...
	  //zeroPricedItems=createTableBody(tbody,currentList,true);
		...
		if(i>0 && i != 3){
			var cell = document.createElement('td');
	    	var cellContent = " "; // todo make this a real value
			if(localStoreList.indexOf(currentList[x][0]) != -1) {
				var cellContent = "*"; // todo make this a real value
			}
...
function createDocWin(evt) {
	...
	if(evt.target.name) {
		...
		// marty-6 try to force new window to our domain (else get security error)
		var readoutWindow = window.open("","_blank","scrollbars=1,resizable=1,status=1,width=600,height=600");
		var dumb = readoutWindow.document;
...
//		if(rows[i].cells[2].textContent=="0") {
		if(rows[i].cells[3].textContent=="0") {
...
function getItemList(callback) {
	...
	// this was broken, don't know why
//	GM_get("kolmafia.svn.sourceforge.net/viewvc/*checkout*/kolmafia/src/data/itemdescs.txt",parseItems);
...
//					items[split[1]]=[split[0],split[2]];
					items[split[2]]=[split[0],split[1]];  // MARTY - Item list format is id, name, descnum
//					if(martytemp < 3) {
//						alert("adding item to database: items["+split[2]+"]=["+split[0]+","+split[1]+"]");
//						martytemp = martytemp+1;
//					}
				}
			}
...
function unstoreItemList(callback) {
	...
	var currentTime = parseInt(new Date().getTime()/60000);
	// temporary bypass since source forge is down, fix it! - MARTY
//	GM_setValue("lastItemUpdate",currentTime);
	
	lastUpdate = GM_getValue("lastItemUpdate",1);
//	lastUpdate=0;// MARTYTEMP

	//check every week.
	if(isEmpty(storedItemList) || (currentTime-lastUpdate)>10080) {
...
//	showNeedUpdate();
	if(GM_getValue('newestVersion',0)*1 > currentVersion) {
		...
		updateLink.href=GM_getValue('scriptLocation','http://sites.google.com/site/pricegunrepository/files/pricegun2.user.js');
...
function getSkills(){
	...
	function parseSkills(skillList){
		var skillMap = new Object();
		var a=skillList.split('\r\n');
		for(var i=0;i<a.length;i++){
			//for each line of file
			var split = a[i].split('\t');
			var currentskill = new Object();
...
function getFamiliars(){
	...
	function parseFamiliars(familiarList){
		var familiarMap = new Object();
		var a=familiarList.split('\r\n');
		for(var i=0;i<a.length;i++){
			//for each line of file
			var split = a[i].split('\t');
			var currentfamiliar = new Object();
...
function getPriceList(callback) {
	status("retrieving version information");
	getVersion();
	
	status("retrieving skill information");
	getSkills();
	
	status("retrieving familiar information");
	getFamiliars();
	
	var tempPrices = new Object();
	
	status("retrieving new price list from " + dataLocation);
	if(/^http:\/\//.test(dataLocation)){
		GM_get(dataLocation.slice(7),parsePrices);
	}
	else if(/^https:\/\//.test(dataLocation)){
		GM_get(dataLocation.slice(8),parsePrices);
	}
	else{
	...
	function parsePrices(pricelist) {
		...
		// marty add for mafia price lsit
		else if(useMafiaPriceList) {
			var a=pricelist.split('\n');
//	alert("hit the marty mafia price code - length is " + a.length);
			for(var i=1;i<a.length;i++){
				//for each line of file - skip revision number line
				var split = a[i].split('\t');
		
				if(/[0-9]/.test(a[i].charAt(0))) {
					// Normal price
					if(split.length==3) {  // mafia price list is itemID, descID, price - skip description
						if(/99999999[0-9]/.test(split[2]))
							tempPrices[split[0]]=['0',false];  // bogus prices 999,999,99X
						else
							tempPrices[split[0]]=[split[2],false];
					}
				}
			}
			GM_setValue('storedPriceList',tempPrices.toSource());
...
function unstorePriceList(callback) {
	// Note: Also gets fmailiar and skill lists as well.
...
//	GM_setValue("lastPriceUpdate",currentTime);
	...
	if(isEmpty(storedPriceList) || (currentTime-lastUpdate)>1440) {
...
// marty - fake function overload so can specify a list to parse if not the default 'whichitem1'
function parseFromForm(formname,sectionTotal,sectionList) {
	return parseFromForm1(formname, 'whichitem1', sectionTotal,sectionList);
}

function parseFromForm1(formname, selectname, sectionTotal,sectionList) {
	var fs,f,opts;
//	if((fs=parser.getElementsByTagName('form')) && (f=fs.namedItem(formname)) && (opts=f.elements.namedItem('whichitem1').options)) {
	if((fs=parser.getElementsByTagName('form')) && (f=fs.namedItem(formname)) && f.elements.namedItem(selectname) && (opts=f.elements.namedItem(selectname).options)) {
